...
def generate(
    ...
    for i in range(minimum_number_of_spans, maximum_number_of_spans):
        # Generate Left Ground
        elements = [utils.generate_ground_element("left-ground")]
        relationships = [{
            "name": "left-ground-deck-{0}".format(minimum_number_of_spans - 1),
            "type": "boundary",
            "elements": [
                {"name": "left-ground"},
                {"name": "deck-{0}".format(minimum_number_of_spans - 1)}
            ]
        }]
        # Setup N^(S_Min-1) Node
        elements.append({
            "name": "deck-{0}".format(minimum_number_of_spans - 1),
            "type": "regular",
        ...
        relationships.append({
            "name": "deck-{0}-deck-{1}".format(minimum_number_of_spans - 1, minimum_number_of_spans),
            "type": "perfect",
            "elements": [
                {"name": "deck-{0}".format(minimum_number_of_spans - 1)},
                {"name": "deck-{0}".format(minimum_number_of_spans)}
            ]
        })
        # Generate Spans
        for j in range(0, i):
        ...
        # Setup N^(S_Max+1) Node
        elements.append({
            "name": "deck-{0}".format(i + 1),
            "type": "regular",
            "contextual": {"type": "deck"}
        })
        relationships.append({
            "name": "deck-{0}-deck-{1}".format(i, i + 1),
            "type": "perfect",
            "elements": [
        ...
        relationships.append({
            "name": "deck-{0}-right-ground".format(i + 1),
            "type": "boundary",
            "elements": [
                {"name": "deck-{0}".format(i + 1)},
                {"name": "right-ground"}
        ...
        utils.generate_document_and_save(
            "slab-bridge-interim-canonical-form-{0}-span-1-column".format(i),
            "Slab Bridge Interim Canonical Form - {0} span".format(i),
            elements,
            relationships
        )
        # Save Graph
